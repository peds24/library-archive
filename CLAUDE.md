...
A Google lookup costs **two** requests, and must stay that way: the `q=isbn:` search returns a *projection* of `volumeInfo`, not the whole record, and it reports missing fields as `0` rather than omitting them — so `pageCount ?? 0` cannot catch it and books land with 0 pages (measured: ISBN 9781534332560 reports 0 in search, 536 in its own record). `fetchVolumeRecord` re-fetches `/volumes/{id}` for the real metadata and returns null on failure so the search payload still serves as a fallback.
...
There is a **third** bad image the dimension checks cannot see, so `isGooglePlaceholder` runs before them: Google only re-renders a cover at raised zoom for volumes it has actually scanned, and answers every other zoom above 1 with a grey "image not available" card (300x391, 575x750, 800x1043, 1280x1670 — all aspect ~0.767, all comfortably over the width floor). It is detected by content type, since Google serves real covers as JPEG and the card as PNG, and the check is scoped by URL to Google's content renderer so a legitimately-PNG cover from elsewhere is untouched. This is why `coverCandidates` ends with Google's own `zoom=1` — for an unscanned volume that is the only real art Google has.
